...
export const ReservationModal: React.FC<ReservationModalProps> = ({ isOpen, onClose }) => {
  ...
  const [needsBeachTable, setNeedsBeachTable] = useState(true);
  const [locationPref, setLocationPref] = useState<'beira-mar' | 'area-coberta'>('beira-mar');
  ...
  const handleSendReservation = (e: React.FormEvent) => {
    ...
    let text = `*🏖️ SOLICITAÇÃO DE RESERVA / MESA - FOOD TIME*\n`;
    text += `------------------------------------\n`;
    text += `👤 *Nome:* ${name.trim()}\n`;
      ...
      <div className="relative w-full max-w-lg bg-amber-50 rounded-[2.5rem] border-4 border-amber-400 shadow-2xl p-6 sm:p-8 z-10 space-y-6 max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-orange-600 font-bold mt-1">
                Garante sua sombra e atendimento na Praia do Francês
              </p>
            </div>
          </div>
        ...
        <form onSubmit={handleSendReservation} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                className="w-full px-3 py-2.5 rounded-2xl border-2 border-amber-200 focus:border-orange-500 focus:outline-none text-xs font-bold bg-white"
              >
                <option value="beira-mar">Pé na Areia (Beira-mar)</option>
                <option value="area-coberta">Área Coberta / Salão</option>
              </select>
            </div>
          </div>
          ...
          <div className="bg-white p-3.5 rounded-2xl border-2 border-amber-300 flex items-center justify-between">
            ...
            <input
              type="checkbox"
              checked={needsBeachTable}
              onChange={(e) => setNeedsBeachTable(e.target.checked)}
          ...
          <div>
            ...
            <textarea
              rows={2}
              placeholder="Ex: Preferência por guarda-sol na frente da água..."
